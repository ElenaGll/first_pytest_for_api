...
class TestUserAuth:
    ...
    def test_user_auth(self):
        ...
        assert "auth_sid" in response1.cookies, \
            "There is no auth cookie in the response"
        assert "x-csrf-token" in response1.headers, \
            "There is no CSRF token header in the response"
        assert "user_id" in response1.json(), \
            "There is no user id in the response"
        ...
        assert user_id_from_auth_method == user_id_from_chek_method, \
